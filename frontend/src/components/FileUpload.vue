...
const MAX_SIZE_MB = 100; // Balanced limit for eBooks (technical books with images)
...
<template>
  <div class="bg-white rounded-lg shadow p-6">
    ...
    <!-- Upload Progress -->
    <div v-if="uploadProgress" class="mt-4 space-y-3">
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center space-x-2">
            <svg
              v-if="uploadProgress.stage === 'uploading'"
            ...
            <svg
              v-else
              class="animate-pulse h-5 w-5 text-blue-600"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <span class="text-sm font-medium text-blue-900">{{ uploadProgress.statusText }}</span>
          </div>
          <span class="text-xs font-semibold text-blue-700">
            {{ uploadProgress.stage === 'uploading' ? Math.round(uploadProgress.uploadProgress) + '%' : '...' }}
          </span>
        </div>
        ...
        <div class="w-full bg-blue-200 rounded-full h-2.5 overflow-hidden">
          <div
            ...
            :class="uploadProgress.stage === 'uploading' ? 'bg-blue-600' : 'bg-blue-600 animate-pulse'"
            :style="{ width: uploadProgress.stage === 'uploading' ? `${uploadProgress.uploadProgress}%` : '100%' }"
          ></div>
        </div>

        <!-- File name -->
        <p class="text-xs text-blue-700 mt-2 truncate">{{ uploadProgress.fileName }}</p>
      </div>
    </div>

    <div v-if="selectedFiles.length > 0 && !uploadProgress" class="mt-4">
